...
class PositionSizing(BaseModel):
    ...
    dollar_amount: Optional[float] = Field(None, gt=0, description="Dollar amount to invest")
    percentage_of_portfolio: Optional[float] = Field(None, gt=0, le=100, description="Percentage of portfolio")
    risk_amount: Optional[float] = Field(None, gt=0, description="Maximum risk amount in dollars")
    risk_percentage: Optional[float] = Field(None, gt=0, le=100, description="Maximum risk as percentage")
    kelly_fraction: Optional[float] = Field(None, gt=0, le=1, description="Kelly criterion fraction")
    volatility_adjusted: bool = Field(False, description="Whether sizing is volatility adjusted")
    
    @root_validator
    def validate_sizing_method(cls, values):
        """Ensure at least one sizing method is specified."""
        sizing_fields = ['shares', 'dollar_amount', 'percentage_of_portfolio', 'risk_amount']
...
class RiskManagement(BaseModel):
    ...
    stop_loss_percentage: Optional[float] = Field(None, gt=0, le=100, description="Stop loss as percentage from entry")
    trailing_stop: Optional[float] = Field(None, gt=0, description="Trailing stop distance")
    trailing_stop_percentage: Optional[float] = Field(None, gt=0, le=100, description="Trailing stop percentage")
    
    # Advanced risk management
    time_stop: Optional[datetime] = Field(None, description="Time-based stop")
    volatility_stop: Optional[float] = Field(None, gt=0, description="Volatility-based stop multiplier")
    drawdown_stop_percentage: Optional[float] = Field(None, gt=0, le=100, description="Maximum drawdown stop")
    
    # Risk metrics
    max_risk_per_trade: Optional[float] = Field(None, gt=0, le=100, description="Maximum risk per trade percentage")
    risk_reward_ratio: Optional[float] = Field(None, gt=0, description="Target risk-reward ratio")
    win_rate_required: Optional[float] = Field(None, gt=0, le=1, description="Required win rate for profitability")


class TargetPrices(BaseModel):
    """Target price levels and management."""
    primary_target: Optional[float] = Field(None, gt=0, description="Primary target price")
    secondary_targets: List[float] = Field(default_factory=list, description="Additional target levels")
    
    # Target management
    scale_out_levels: List[float] = Field(default_factory=list, description="Scale out price levels")
    scale_out_percentages: List[float] = Field(default_factory=list, description="Scale out percentages at each level")
    
    # Target calculations
    measured_move_target: Optional[float] = Field(None, gt=0, description="Measured move target price")
    fibonacci_targets: List[float] = Field(default_factory=list, description="Fibonacci extension targets")
    support_resistance_targets: List[float] = Field(default_factory=list, description="Support/resistance targets")
...
class ExecutionParameters(BaseModel):
    ...
    order_type: str = Field("market", description="Order type (market, limit, stop, etc.)")
    ...
    # Advanced execution
    iceberg_size: Optional[int] = Field(None, gt=0, description="Iceberg order visible size")
    minimum_fill_size: Optional[int] = Field(None, gt=0, description="Minimum fill size")
    all_or_none: bool = Field(False, description="All or none execution")
    
    # Timing
    execution_window_start: Optional[datetime] = Field(None, description="Earliest execution time")
    execution_window_end: Optional[datetime] = Field(None, description="Latest execution time")
    market_session: str = Field("regular", description="Market session (regular, pre, post, extended)")
...
class SignalMetrics(BaseModel):
    ...
    quality_score: float = Field(..., ge=0, le=1, description="Overall signal quality (0-1)")
    
    # Historical performance
    historical_success_rate: Optional[float] = Field(None, ge=0, le=1, description="Historical success rate")
    average_return: Optional[float] = Field(None, description="Average return from similar signals")
    average_holding_period_days: Optional[float] = Field(None, gt=0, description="Average holding period")
    
    # Pattern-specific metrics
    pattern_reliability: Optional[float] = Field(None, ge=0, le=1, description="Pattern reliability score")
    market_condition_score: Optional[float] = Field(None, ge=0, le=1, description="Market condition favorability")
    sector_momentum_score: Optional[float] = Field(None, ge=0, le=1, description="Sector momentum score")
    
    # Risk metrics
    sharpe_ratio: Optional[float] = Field(None, description="Expected Sharpe ratio")
    maximum_adverse_excursion: Optional[float] = Field(None, description="Maximum adverse excursion")
    maximum_favorable_excursion: Optional[float] = Field(None, description="Maximum favorable excursion")
...
class TradingSignal(BaseModel):
    ...
    status: SignalStatus = Field(SignalStatus.PENDING, description="Current signal status")
    priority: SignalPriority = Field(SignalPriority.MEDIUM, description="Signal priority")
    
    # Source Information
    source_pattern_id: Optional[str] = Field(None, description="Source pattern ID if applicable")
    ...
    supporting_timeframes: List[str] = Field(default_factory=list, description="Supporting timeframes")
    ...
    position_sizing: PositionSizing = Field(..., description="Position sizing parameters")
    
    # Execution
    execution_params: ExecutionParameters = Field(..., description="Execution parameters")
    
    # Signal Quality and Metrics
    metrics: SignalMetrics = Field(..., description="Signal quality metrics")
    
    # Market Context
    market_context: Dict[str, Any] = Field(default_factory=dict, description="Market conditions at signal generation")
    technical_indicators: Dict[str, float] = Field(default_factory=dict, description="Technical indicator values")
    
    # Timeline
    generated_at: datetime = Field(default_factory=datetime.now, description="When signal was generated")
    valid_until: Optional[datetime] = Field(None, description="Signal expiry time")
    executed_at: Optional[datetime] = Field(None, description="When signal was executed")
    ...
    realized_return: Optional[float] = Field(None, description="Realized return percentage")
    unrealized_return: Optional[float] = Field(None, description="Current unrealized return percentage")
    max_favorable_excursion: Optional[float] = Field(None, description="Maximum favorable move")
    max_adverse_excursion: Optional[float] = Field(None, description="Maximum adverse move")
    ...
    def update_status(self, new_status: SignalStatus, timestamp: Optional[datetime] = None):
        ...
        elif new_status in [SignalStatus.TARGET_HIT, SignalStatus.STOPPED_OUT] and not self.closed_at:
    ...
    def is_active(self) -> bool:
        ...
        return (self.status in [SignalStatus.PENDING, SignalStatus.ACTIVE, SignalStatus.EXECUTED, 
                               SignalStatus.FILLED, SignalStatus.PARTIAL_FILL] and not self.is_expired())
    ...
    def calculate_risk_reward_ratio(self) -> Optional[float]:
        ...
        if not all([self.entry_price, self.targets.primary_target, self.risk_management.stop_loss]):
            return None

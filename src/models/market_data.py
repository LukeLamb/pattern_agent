...
class MarketData(BaseModel):
    ...
    timeframe: str = Field(
        ..., description="Data timeframe (1min, 5min, 1hr, daily, etc.)"
    )
    data_type: MarketDataType = Field(
        MarketDataType.OHLCV, description="Type of market data"
    )
    ...
    market_session: MarketSession = Field(
        MarketSession.REGULAR, description="Market session"
    )

    # Quality metrics
    completeness_score: float = Field(
        1.0, ge=0, le=1, description="Data completeness (0-1)"
    )
    ...
    total_volume: Optional[int] = Field(
        None, description="Total volume across all periods"
    )
    ...
    @classmethod
    def from_dataframe(
        cls, df: pd.DataFrame, symbol: str, timeframe: str, **kwargs
    ) -> "MarketData":
    ...
    def calculate_vwap(self) -> float:
        ...
        total_pv = sum(
            ohlcv.volume * ((ohlcv.high + ohlcv.low + ohlcv.close) / 3)
            for ohlcv in self.data
        )
    ...
    def validate_data_quality(self) -> Dict[str, Any]:
        ...
        completeness = (
            1.0 - (missing_periods / expected_periods) if expected_periods > 0 else 1.0
        )
        ...
        return {
            ...
            "quality_score": max(
                0.0, completeness - (len(gaps) * 0.01) - (len(anomalies) * 0.02)
            ),
        }
    ...
    def _calculate_expected_periods(self) -> int:
        ...
        period_seconds = timeframe_seconds.get(
            self.timeframe, 3600
        )  # Default to 1 hour
    ...
    def _detect_price_anomalies(self) -> List[int]:
        ...
        for i, ohlcv in enumerate(self.data):
            if (
                abs(ohlcv.close - mean_price) > threshold
                or abs(ohlcv.high - mean_price) > threshold
                or abs(ohlcv.low - mean_price) > threshold
            ):
                anomalies.append(i)
    ...
    def get_summary_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "price_change_percent": (
                (self.data[-1].close - self.data[0].close) / self.data[0].close
            )
            * 100,
            ...
            "volatility": (
                (sum((r / 100) ** 2 for r in returns) / len(returns)) ** 0.5 * 100
                if returns
                else 0
            ),
        }

...
class SupportResistanceLevel(BaseModel):
    ...
    volume_confirmation: float = Field(0.0, ge=0, description="Average volume at this level")
...
class PatternMetrics(BaseModel):
    ...
    formation_duration_days: float = Field(..., gt=0, description="Days taken to form pattern")
    price_range_percent: float = Field(..., gt=0, description="Price range as percentage of entry price")
    volume_trend: float = Field(..., description="Volume trend during formation (-1 to 1)")
    volatility_normalized: float = Field(..., ge=0, description="Volatility normalized to historical average")
    symmetry_score: float = Field(0.0, ge=0, le=1, description="Pattern symmetry score (0-1)")
    breakout_strength: Optional[float] = Field(None, ge=0, description="Breakout strength if applicable")


class ValidationCriteria(BaseModel):
    """Pattern validation criteria and results."""
    min_touches: int = Field(2, ge=1, description="Minimum touches required for pattern")
    min_duration_days: float = Field(1.0, gt=0, description="Minimum formation duration")
    max_duration_days: float = Field(365.0, gt=0, description="Maximum formation duration")
    min_price_range_percent: float = Field(1.0, gt=0, description="Minimum price range percentage")
    volume_confirmation_required: bool = Field(True, description="Whether volume confirmation is required")
    volume_threshold_multiplier: float = Field(1.5, gt=0, description="Volume multiplier for confirmation")
    
    # Validation results
    touches_validated: bool = Field(False, description="Whether touch count is validated")
    ...
    price_action_validated: bool = Field(False, description="Whether price action is validated")
    overall_validated: bool = Field(False, description="Overall validation status")
    validation_score: float = Field(0.0, ge=0, le=1, description="Composite validation score")
    validation_reasons: List[str] = Field(default_factory=list, description="Validation failure reasons")
...
class Pattern(BaseModel):
    ...
    symbol: str = Field(..., min_length=1, description="Trading symbol (e.g., AAPL, BTCUSD)")
    pattern_type: PatternType = Field(..., description="Type of pattern detected")
    status: PatternStatus = Field(PatternStatus.FORMING, description="Current pattern status")
    ...
    completion_date: Optional[datetime] = Field(None, description="When pattern completed formation")
    breakout_date: Optional[datetime] = Field(None, description="When breakout occurred")
    expiry_date: Optional[datetime] = Field(None, description="When pattern expires if not confirmed")
    
    # Timeframe
    timeframe: str = Field(..., description="Primary timeframe (e.g., '1hr', 'daily')")
    additional_timeframes: List[str] = Field(default_factory=list, description="Supporting timeframes")
    ...
    target_prices: List[float] = Field(default_factory=list, description="Target price levels")
    stop_loss: Optional[float] = Field(None, gt=0, description="Stop loss price")
    
    # Pattern Metrics
    confidence_score: float = Field(..., ge=0, le=1, description="Pattern confidence (0-1)")
    strength_score: float = Field(..., ge=0, le=1, description="Pattern strength (0-1)")
    reliability_score: float = Field(..., ge=0, le=1, description="Historical reliability (0-1)")
    pattern_metrics: PatternMetrics = Field(..., description="Detailed pattern metrics")
    
    # Validation
    validation_criteria: ValidationCriteria = Field(..., description="Validation criteria and results")
    
    # Market Context
    market_context: Dict[str, Any] = Field(default_factory=dict, description="Market conditions during formation")
    sector_context: Dict[str, Any] = Field(default_factory=dict, description="Sector-specific context")
    
    # Risk Assessment
    risk_reward_ratio: Optional[float] = Field(None, gt=0, description="Calculated risk-reward ratio")
    max_risk_percent: Optional[float] = Field(None, gt=0, le=100, description="Maximum risk as percentage")
    probability_success: Optional[float] = Field(None, ge=0, le=1, description="Success probability based on history")
    
    # Technical Data
    formation_data: Dict[str, Any] = Field(default_factory=dict, description="Raw formation data and calculations")
    indicator_values: Dict[str, float] = Field(default_factory=dict, description="Technical indicator values at detection")
    
    # Metadata
    created_at: datetime = Field(default_factory=datetime.now, description="When pattern record was created")
    updated_at: datetime = Field(default_factory=datetime.now, description="When pattern record was last updated")
    ...
    def validate_risk_reward(self):
        ...
        if entry_price and target_prices and stop_loss:
            ...
            potential_profit = abs(target_prices[0] - entry_price) if target_prices else 0
    ...
    def update_status(self, new_status: PatternStatus, timestamp: Optional[datetime] = None):
    ...
    def is_active(self) -> bool:
        ...
        return self.status not in [PatternStatus.FAILED, PatternStatus.EXPIRED] and not self.is_expired()
    ...
    def add_support_level(self, price: float, strength: int, touches: int, last_touch: datetime):
    ...
    def add_resistance_level(self, price: float, strength: int, touches: int, last_touch: datetime):

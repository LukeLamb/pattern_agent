...
try:
    from ..pattern_detection.pattern_engine import (
        DetectedPattern, PatternType, PatternStrength,
        PivotPoint, SupportResistanceLevel, TrendLine
    )
...
class PatternValidator:
    ...
    def validate_pattern(self, pattern: DetectedPattern, 
                        market_data: MarketData) -> List[ValidationResult]:
        """
    ...
    def calculate_overall_validation_score(self, validation_results: List[ValidationResult]) -> float:
        """
    ...
    def calculate_pattern_quality_metrics(self, pattern: DetectedPattern,
                                         market_data: MarketData) -> PatternQualityMetrics:
        """
    ...
    def _validate_volume_confirmation(self, pattern: DetectedPattern, 
                                    market_data: MarketData) -> ValidationResult:
        ...
        pattern_data = df[(df.index >= pattern.start_time) & (df.index <= pattern.end_time)]
        ...
        if len(pattern_data) > 0:
            ...
            if pattern_start_pos >= pattern_length:
                pre_pattern_data = df.iloc[pattern_start_pos-pattern_length:pattern_start_pos]
                pre_pattern_volume = pre_pattern_data['volume'].mean()
            else:
                pre_pattern_data = df.iloc[:pattern_start_pos]
                pre_pattern_volume = pre_pattern_data['volume'].mean() if len(pre_pattern_data) > 0 else pattern_volume
        else:
        ...
        volume_ratio = pattern_volume / pre_pattern_volume if pre_pattern_volume > 0 else 1.0
    ...
    def _validate_timeframe_consistency(self, pattern: DetectedPattern,
                                      market_data: MarketData) -> ValidationResult:
        """Validate timeframe consistency for pattern formation."""
        # Calculate pattern formation duration
        formation_duration = (pattern.end_time - pattern.start_time).days
        ...
        passed = bool(self.min_formation_days <= formation_duration <= self.max_formation_days)
    ...
    def _validate_historical_success_rate(self, pattern: DetectedPattern) -> ValidationResult:
        ...
        if len(historical_patterns) < 5:
            ...
            successful_patterns = sum(1 for p in historical_patterns if p.get('success', False))
    ...
    def _validate_pattern_quality(self, pattern: DetectedPattern,
                                 market_data: MarketData) -> ValidationResult:
    ...
    def _validate_market_context(self, pattern: DetectedPattern,
                                market_data: MarketData) -> ValidationResult:
        ...
        trend_strength = abs(sma_20.iloc[-1] - sma_50.iloc[-1]) / sma_50.iloc[-1] if len(df) >= 50 else 0.02
    ...
    def _calculate_formation_time_score(self, pattern: DetectedPattern) -> float:
        ...
        formation_days = (pattern.end_time - pattern.start_time).days
    ...
    def _calculate_symmetry_score(self, pattern: DetectedPattern) -> float:
        ...
        elif pattern.pattern_type in [PatternType.ASCENDING_TRIANGLE, 
    ...
    def _calculate_volume_pattern_score(self, pattern: DetectedPattern, df: pd.DataFrame) -> float:
        ...
        if pattern.pattern_type in [PatternType.ASCENDING_TRIANGLE, 
                                  PatternType.DESCENDING_TRIANGLE,
                                  PatternType.SYMMETRICAL_TRIANGLE]:
    ...
    def _calculate_technical_strength_score(self, df: pd.DataFrame) -> float:
        ...
        divergence_score = 1.0 if (price_trend > 0 and rsi_trend < 0) or (price_trend < 0 and rsi_trend > 0) else 0.6
        
        return min(1.0, max(0.2, divergence_score))
    
    def _calculate_market_context_score(self, pattern: DetectedPattern, df: pd.DataFrame) -> float:
    ...
    def _score_volatility_for_pattern(self, pattern: DetectedPattern, volatility: float) -> float:
    ...
    def _score_trend_for_pattern(self, pattern: DetectedPattern, trend_strength: float, df: pd.DataFrame) -> float:
        ...
        if pattern.pattern_type == PatternType.ASCENDING_TRIANGLE:
            ...
            return 1.0 if trend_strength > 0.02 and df['close'].iloc[-1] > df['close'].iloc[-20] else 0.6

...
def demonstrate_pattern_validation():
    ...
    print(
        f"   Time range: {market_data.start_time.date()} to {market_data.end_time.date()}"
    )

    # 2. Initialize pattern detection engine
    print("\n2. Initializing pattern detection engine...")
    detection_engine = PatternDetectionEngine(min_pattern_length=10, min_touches=2)
    ...
    for i, pattern in enumerate(detected_patterns):
        print(
            f"   [{i+1}] {pattern.pattern_type.value} - Confidence: {pattern.confidence_score:.2f}"
        )
        print(
            f"       Period: {pattern.start_time.date()} to {pattern.end_time.date()}"
        )
    ...
    for i, pattern in enumerate(detected_patterns):
        ...
        quality_metrics = validator.calculate_pattern_quality_metrics(
            pattern, market_data
        )
        ...
        for result in validation_results:
            ...
            if result.criteria == ValidationCriteria.VOLUME_CONFIRMATION:
                ...
                print(
                    f"     Volume Ratio: {volume_ratio:.2f} (threshold: {result.details.get('threshold', 'N/A')})"
                )
                ...
                print(
                    f"     Historical Success Rate: {success_rate:.2f} ({historical_count} samples)"
                )
        ...
        print(
            f"   • Technical Strength Score: {quality_metrics.technical_strength_score:.3f}"
        )
        ...
        adjusted_confidence = (
            original_confidence * quality_metrics.confidence_adjustment
        )
    ...
    if detected_patterns:
        high_quality_patterns = sum(
            ...
            if validator.calculate_overall_validation_score(
                validator.validate_pattern(pattern, market_data)
            )
            >= 0.7
        )
